...
const onSubmit = handleSubmit((values) => loginHandler(values))

async function loginHandler(values) {
  await userStore.loginUser(values.email, values.password).catch((error) => {
...
<template>
  <div class="flex mt-5 justify-content-center">
    <Card class="w-full lg:max-w-22rem mt-5">
      <template #header>
        <h1 class="px-5 pt-5 mb-0 text-center">Login</h1>
      </template>
      <template #content>
        <form novalidate @submit.prevent="onSubmit" class="flex flex-column">
          ...
          <Button type="submit" :disabled="userStore.loadingUser" label="Submit" class="mt-4" />
        </form>
      </template>
    </Card>
    <Toast />
